  ...
  module Serverside
    class DeployBase < Task
      ...
      def parse_configured_services
        YAML.load_file "#{c.shared_path}/config/ey_services_config_deploy.yml"
      rescue
        {}
      end
      ...
      def check_for_ey_config
        if gemfile? && lockfile
          ...
          if !configured_services.empty? && !lockfile.has_ey_config?
            message = "Gemfile.lock does not contain ey_config. Add it to get access to:"
            configured_services.each do |service_label, vars|
              vars.each do |k, _|
                message.puts "EY::Config.get(:#{service_label},'#{k}')"
              end
            end
            warning << message
          end
